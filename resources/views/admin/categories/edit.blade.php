    ...
    <x-ui.card title="Category Details">
        <form action="{{ route('admin.categories.update', $category) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="space-y-1.5">
                ...
                <textarea name="description" id="description" rows="4" class="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="Briefly describe this category...">{{ $category->description }}</textarea>
            </div>
            ...
            <div class="space-y-1.5">
                ...
                @if($category->image)
                    <div class="mb-4">
                        <img src="{{ Storage::url($category->image) }}" alt="{{ $category->name }}" class="h-24 w-24 object-cover rounded-xl border border-gray-200">
                        <p class="text-xs text-gray-400 mt-1 italic">Current image</p>
                    </div>
                @endif
            ...
            <div class="flex items-center">
                <input id="is_active" name="is_active" type="checkbox" {{ $category->is_active ? 'checked' : '' }} class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded">
            ...
            <div class="pt-4">
                <x-ui.button type="submit" class="w-full" size="lg">Update Category</x-ui.button>
            </div>
        </form>
    </x-ui.card>
</div>
